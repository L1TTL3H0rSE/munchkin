...
task document. Блоки «Почему» и «Пример» объясняют правило, но сами не
...
wireflows принадлежат отдельной `ui.md`. Этот документ определяет
...
Ослабление требует обоснованного решения и доказанного incompatible dependency.
...
10. consumer build и clean diff.
...
```markdown
- [ ] Authorized scope and component contracts remain accurate.
...
- [ ] pnpm build:local, pnpm lint, pnpm check, pnpm build:storybook и pnpm build прошли.
...
4. responsive gameplay layout по `ui.md`;
