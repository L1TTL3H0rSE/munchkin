...
const gameID = ref("");
const localError = ref<LobbyFormError>();
const formError = computed(() => localError.value ?? props.error);
const state = computed<LobbyFormState>(() => {
  if (props.busy) { return "loading"; }
  if (formError.value) { return formError.value.kind === "offline" ? "offline" : "error"; }
  return props.successful ? "success" : "idle";
});
...
async function focusField(field: LobbyField): Promise<void> {
  await nextTick();
  if (field === "gameID") { gameIDInput.value?.focus(); return; }
  displayNameInput.value?.focus();
}

async function setError(error: LobbyFormError): Promise<void> {
  localError.value = error;
  await focusField(error.field);
}

async function handleInvalid(event: Event): Promise<void> {
...
async function handleSubmit(): Promise<void> {
  ...
  if (validationError) {
    await setError(validationError);
    return;
  }
...
watch(() => props.error, async (error) => {
  if (error) { await focusField(error.field); }
});
</script>
...
.lobby-form {
  ...
  &[data-state="loading"] { border-color: var(--color-accent-strong); }
  &[data-state="success"] { border-color: var(--color-success); }
  &[data-state="offline"],
  ...
  h2 { margin: 0; font-size: clamp(1.45rem, 3vw, 1.9rem); }

  &__description { min-height: 2.8em; margin: 0; color: var(--color-text-muted); }
  ...
  &__legend { position: absolute; width: 1px; height: 1px; overflow: hidden; clip: rect(0 0 0 0); white-space: nowrap; }
  label { display: grid; gap: .15rem; color: var(--color-text-muted); }
  input { min-height: 44px; margin-top: .2rem; }
  input[aria-invalid="true"] { border-color: var(--color-danger); }

  &__field-error,
  &__error,
  &__status { margin: -.45rem 0 0; font-size: .9rem; line-height: 1.4; }
  ...
  &__status { color: var(--color-success); }
  ...
  &--compact {
    ...
    .lobby-form__fields { gap: 12px; }
    label { gap: 6px; color: var(--color-text-secondary); font-size: 10px; }
    input { min-height: 48px; margin-top: 0; border-radius: var(--radius-control); padding: 12px 14px; background: var(--color-surface-control); font-size: 14px; }
    ...
    .lobby-form__submit {
      ...
      &:disabled { border-color: var(--color-line); color: var(--color-surface); background: color-mix(in srgb, var(--color-accent) 30%, var(--color-surface)); }
    }

    .lobby-form__error,
    .lobby-form__status,
    .lobby-form__field-error { margin: 0; font-size: 10px; }
  }
}
...
@include api.from(tablet-large) {
  .lobby-form--compact { gap: 16px; }
}

@media (max-width: 374px) {
  .lobby-form:not(.lobby-form--compact) { padding: 1rem; }
