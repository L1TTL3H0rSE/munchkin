...
export function lobbyErrorForKind(kind: GameApiErrorKind): LobbyFormError {
  const field = kind === "not_found" ? "gameID" : "form";
  const message = lobbyErrorMessage(kind);

  return {
    field,
    kind: kind,
    message,
    retryable: kind === "offline" || kind === "transient",
  };
}
...
export interface LobbySubmissionState {
  busy: boolean;
